    ...
    s3.headBucket({ Bucket }, (err) => {
      if (err) {
        if (err.code === 'NotFound') {
          ...
          reject(new Error(`
            Bucket "${Bucket}" exists, but you do not have permission to access it.
            NOTE: S3 buckets must be unique. Ensure the bucket you specified has a unique name.
          `));
        } else {
          reject(err);
        }
...
module.exports = async function (s3, bucketConfig) {
